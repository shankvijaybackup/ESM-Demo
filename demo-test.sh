...
curl -s -X POST $API -H "Content-Type: application/json" \
  -d '{"text":"Apply for 2 days annual leave from 2026-02-20 to 2026-02-21","employeeId":"EMP001"}' \
...
curl -s -X POST $API -H "Content-Type: application/json" \
  -d '{"text":"Submit reimbursement for $450 travel expense","employeeId":"EMP003"}' \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'   ✓ {d[\"message\"]}')"
echo ""
...
curl -s -X POST $API -H "Content-Type: application/json" \
  -d '{"text":"Mark my attendance","employeeId":"EMP004"}' \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'   ✓ {d[\"message\"]}')"
echo ""
...
curl -s -X POST $API -H "Content-Type: application/json" \
  -d '{"text":"Claim $850 for meal expenses","employeeId":"EMP002"}' \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'   ✓ {d[\"message\"]}')"
echo ""
...
curl -s -X POST $API -H "Content-Type: application/json" \
  -d '{"text":"Create purchase order for $2500 vendor: ABC Textiles items: Cotton fabric","employeeId":"EMP002"}' \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'   ✓ {d[\"message\"]}')"
echo ""
...
curl -s -X POST $API -H "Content-Type: application/json" \
  -d '{"text":"Submit invoice INV-2026-999 vendor: ABC Textiles $2500","employeeId":"EMP002"}' \
  | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'   ✓ {d[\"message\"]}')"
echo ""
